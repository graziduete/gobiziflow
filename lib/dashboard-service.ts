  ...
  private static async calculatePercentagePhasesWithDetails(metric: PaymentMetric, monthYear: string): Promise<Array<{
    ...
    try {
      ...
      let projectsQuery = this.supabase
        .from('projects')
        .select('id, name, budget, status, created_at, updated_at, tenant_id')
      ...
      if (company?.tenant_id) {
        console.log(`🔍 [Mensal] Filtrando projetos por tenant_id: ${company.tenant_id}`)
        ...
        console.log(`🔍 [Mensal] Filtrando projetos sem tenant_id (admin normal/master)`)
