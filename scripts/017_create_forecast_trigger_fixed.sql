-- Script corrigido para implementar trigger automático de previsões
...
BEGIN
  ...
  IF NEW.budget > 0 AND forecast_percentage > 0 THEN
    ...
    RAISE NOTICE 'Previsão criada/atualizada para projeto %: R$ % (%%)', NEW.name, forecast_amount, forecast_percentage;
  ELSE
    ...
    RAISE NOTICE 'Previsão removida para projeto % (status: %)', NEW.name, NEW.status;
  END IF;
