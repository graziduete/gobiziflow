-- Script simplificado para implementar trigger automático de previsões
...
-- 4. Testar o trigger - alterar status de um projeto
UPDATE projects 
SET status = 'planning' 
WHERE id = (SELECT id FROM projects LIMIT 1);

-- 5. Verificar se previsões foram criadas
SELECT 
  pf.month_year,
  pf.forecast_amount,
