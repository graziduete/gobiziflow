...
export default function EditarEstimativaPage() {
  ...
  const fetchEstimativa = async () => {
    try {
      ...
      setFormData({
        ...
        meses_previstos: estimativaData.meses_previstos,
        percentual_imposto: estimativaData.percentual_imposto,
        observacoes: estimativaData.observacoes || ''
      })
      setMesesInput(estimativaData.meses_previstos.toString())
      ...
      const recursosComAlocacoes = await Promise.all(
        (recursosData || []).map(async (recurso) => {
          ...
          console.log('Recurso carregado:', recurso.nome_recurso, 'Alocações:', alocacoesData)
      ...
      console.log('Recursos finais carregados:', recursosComAlocacoes)
      
      // Garantir que todos os recursos tenham alocações para todas as semanas
      const mesesPrevistos = estimativaData.meses_previstos
      const recursosComAlocacoesCompletas = recursosComAlocacoes.map(recurso => {
        const alocacoesCompletas = Array.from({ length: mesesPrevistos * 4 }, (_, i) => {
          const semana = i + 1
          const alocacaoExistente = recurso.alocacoes.find(a => a.semana === semana)
          return {
            semana,
            horas: alocacaoExistente?.horas || 0,
            custo_semanal: (alocacaoExistente?.horas || 0) * recurso.taxa_hora
          }
        })
        
        const total_horas = alocacoesCompletas.reduce((total, aloc) => total + aloc.horas, 0)
        const total_custo = total_horas * recurso.taxa_hora
        
        return {
          ...recurso,
          alocacoes: alocacoesCompletas,
          total_horas,
          total_custo
        }
      })
      
      setRecursos(recursosComAlocacoesCompletas)
      
      // Verificar se os recursos têm alocações
      recursosComAlocacoesCompletas.forEach(recurso => {
        console.log(`Recurso ${recurso.nome_recurso}:`, {
          alocacoes: recurso.alocacoes.length,
  ...
  const updateAlocacao = (recursoId: string, semana: number, horas: number) => {
    console.log('updateAlocacao chamada:', { recursoId, semana, horas })
    setRecursos(recursos.map(recurso => {
      if (recurso.id === recursoId) {
        console.log('Atualizando recurso:', recurso.nome_recurso)
        ...
        console.log('Novos totais:', { total_horas, total_custo })
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">
            Edite os detalhes da estimativa e ajuste os recursos
          </p>
        </div>
      </div>
